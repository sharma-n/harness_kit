...
harness_kit's lower layers and break the layering rule).
...
def default_retryable(exc: BaseException) -> bool:
    ...
    harness_kit cannot name backend exception types (redis/sqlite/qdrant) without
    ...
    retried. Terminal: control-flow exceptions, harness_kit's own semantic errors, and
    ...
    if isinstance(exc, (asyncio.CancelledError, KeyboardInterrupt, SystemExit)):
        return False
    if isinstance(exc, HarnessKitError):
        return False
