...
from harness_kit.agent.events import ToolApprovalRequired
...
from harness_kit.serving.wire import encode_conversation, encode_event
...
def create_app(service: AgentService) -> FastAPI:
    ...
    app = FastAPI(title="harness_kit", lifespan=lifespan)
...
def create_app_from_yaml(path: str = "config.yaml") -> FastAPI:
    """ASGI entrypoint factory: ``uvicorn harness_kit.serving.app:app``-friendly."""
