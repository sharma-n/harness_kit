"""harness_kit exception hierarchy.
...
concerns harness_kit owns: authorization, tool execution, and context budgeting.
"""
...
class HarnessKitError(Exception):
    """Base for all harness_kit-specific errors."""


class UnauthorizedError(HarnessKitError):
    """A user tried to access a conversation/session owned by someone else."""


class ContextOverflowError(HarnessKitError):
    """Tier-0 context (system + current message + tool defs) alone overflows."""


class BudgetExceededError(HarnessKitError):
    """A turn exceeded its per-turn wall-clock budget."""


class StoreWriteError(HarnessKitError):
