"""Config dataclass tree for harness_kit.

The single ``config.yaml`` carries harness_kit's own sections **plus** a nested
...
class StoreRetryConfig:
    ...
    Covers harness_kit's own plain store writes only — the LLM ``invoke`` and embedder
...
class SqliteConfig:
    ...
    url: str = "sqlite+aiosqlite:///harness_kit.db"
...
class MetricsConfig:
    ...
    Only the leaf ``harness_kit.metrics`` module imports ``prometheus_client``;
...
class TelemetryConfig:
    ...
    Only the leaf ``harness_kit.telemetry`` module imports ``langfuse``; every other
...
@dataclass(slots=True)
class HarnessKitConfig:
    """Top-level config. Compose from YAML via ``HarnessKitConfig.from_yaml``."""
    ...
    @classmethod
    def from_yaml(cls, path: str) -> HarnessKitConfig:
        from harness_kit.config.loader import load_yaml

        return load_yaml(cls, path)

    @classmethod
    def from_dict(cls, data: dict) -> HarnessKitConfig:
