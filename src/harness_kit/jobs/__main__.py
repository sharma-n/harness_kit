...
from harness_kit.stores.factory import build_stores
from harness_kit.jobs.dedup import EpisodicDeduplicator
from harness_kit.jobs.resummarize import EpisodicResummarizer

logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s %(message)s")
_log = logging.getLogger("harness_kit.jobs")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="harness_kit offline memory jobs")
...
async def _run(args: argparse.Namespace) -> None:
    cfg = HarnessKitConfig.from_yaml(args.config)
